...
export default function AddMedicationButton() {
  ...
  function reset() {
    setName(''); setGenericName(''); setForm('tablet'); setStrength(''); setStrengthUnit('mg')
    setInstructions(''); setPrescriber(''); setPillCount(''); setRefillThreshold('')
    setColor(MED_COLORS[0]); setFrequency('daily'); setTimes(['08:00']); setReminderEnabled(true)
    setError(null)
  }

  function onFrequencyChange(f: string) {
    setFrequency(f)
    setTimes(DEFAULT_TIMES[f]?.length ? [...DEFAULT_TIMES[f]] : f === 'as_needed' ? [] : ['08:00'])
  }
  ...
  return (
    <>
      ...
      >
        {error && (
          <div className="rounded-xl bg-red-50 text-red-700 text-sm px-4 py-3 border border-red-100">{error}</div>
        )}
        ...
        <Field label="Medication name" required>
          ...
          <datalist id="common-meds">
            {COMMON_MEDICATIONS_SA.map(m => <option key={m} value={m} />)}
          </datalist>
        </Field>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Form">
            <select className="input" value={form} onChange={e => setForm(e.target.value)}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Unit">
            <select className="input" value={strengthUnit} onChange={e => setStrengthUnit(e.target.value)}>
        ...
        {frequency !== 'as_needed' && (
          <Field label="Reminder times">
            <div className="space-y-2">
              ...
              <button type="button" onClick={() => setTimes(ts => [...ts, '08:00'])}
                className="text-brand-900 text-sm font-semibold">+ Add time</button>
            </div>
          </Field>
        )}
