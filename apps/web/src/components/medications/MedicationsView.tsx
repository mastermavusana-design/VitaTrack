...
/**
 * Presentational medications page. Fed by either the SSR read (flag off) or the
 * client-direct read wrapper (flag on). Derives active/inactive + adherence here
 * so both data sources produce identical output.
 */
export default function MedicationsView({
  ...
  const activeMeds   = meds.filter((m: any) => m.is_active)
  const inactiveMeds = meds.filter((m: any) => !m.is_active)
